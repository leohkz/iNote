...
const $btnFinish   = document.getElementById('btnFinish');   // NEW: finish = stop + save
...
// ---- State: IDLE ----
// btnStart: enabled  | btnPause: disabled | btnFinish: disabled | btnReset: enabled
// ---- State: RECORDING ----
// btnStart: disabled | btnPause: enabled  | btnFinish: ENABLED  | btnReset: enabled
// ---- State: PAUSED ----
// btnStart: disabled | btnPause: enabled(resume) | btnFinish: ENABLED | btnReset: enabled

function setUI(state) {
  const rec = (state === 'recording');
  const any = (state !== 'idle');
  $btnStart.disabled    = rec || (state === 'paused');
  $btnPause.disabled    = !any;
  $btnFinish.disabled   = !any;   // KEY FIX: enabled whenever recording OR paused
  $lang.disabled        = rec;
  $dot.className = rec ? 'dot active' : state === 'paused' ? 'dot paused' : 'dot idle';
...
// "結束錄製並儲存" = stop hardware + save note
$btnFinish.addEventListener('click', () => {
  hardStop();
  setUI('idle');
  // small delay so mediaRecorder.onstop fires and audioBlob is ready
  setTimeout(saveNote, 300);
});
...
$btnSummarize.addEventListener('click', async () => {
  ...
  try {
    ...
    showSummary('⚠️ AI 負載過高，使用本地摘要：<br><br>' + localSummarize(text).replace(/\n/g,'<br>'));
  }
...
function localSummarize(text) {
  ...
  const freq = {};
  text.replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g,' ').split(/\s+/).filter(Boolean).forEach(w=>freq[w]=(freq[w]||0)+1);
  ...
  return scored.sort((a,b)=>b.score-a.score).slice(0,topN).sort((a,b)=>a.i-b.i).map(x=>x.s.trim()).join('\n');
}
...
function saveNote() {
  ...
  const summary = (aiText && !aiText.startsWith('⚠️') && !aiText.includes('AI 處理中')) ? aiText : '';
  ...
  const persist = (withAudio) => {
    ...
    const msg = withAudio ? '✅ 筆記已儲存！（含錄音）' : '✅ 筆記已儲存！';
    alert(msg);
    fullText = ''; subtitles = []; audioBlob = null;
...
function openNote(note) {
  ...
  // Fresh audio element each time to avoid listener stacking
